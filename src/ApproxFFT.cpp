...
// https://github.com/Bodmer/Audio-Spectrum-FFT/blob/main/Audio_FFT_Display/ApproxFFT.ino
...
float approx_FFT(int in[], unsigned int N, float Frequency)
{
  ...
  // clip the number of samples to power of 2
  a = Pow2[o];
  // Serial.print("o: ");
  // Serial.print(o);
  // Serial.print("  a: ");
  ...
  // getting min max and average for scalling
  for (int i = 0; i < a; i++)
  {
    out_r[i] = 0;
    out_im[i] = 0;
  ...
  // Serial.print("Pow2[o - 1]");
  // Serial.println(Pow2[o - 1]);
  // Serial.print("fstep");
  // Serial.println(fstep);
  // Serial.println("Amplitudes:");
  for (unsigned int i = 1; i < Pow2[o - 1]; i++) // getting amplitude from compex number
  {
    ...
    // Serial.print(i * fstep);
    // Serial.print("Hz \t");
    // Serial.println(out_r[i] << scale);
    // in[i - 1] = out_r[i];
    in[i - 1] = out_r[i] << scale;
  }
  ...
  fb = out_r[fm];
  fc = out_r[fm + 1];
  float ff = (fa * (fm - 1) + fb * fm + fc * (fm + 1)) / (fa + fb + fc);

  return (ff * Frequency / N);
}