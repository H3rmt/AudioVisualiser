...
namespace AnimationsUtil {
    inline void clearRgbPixels(uint32_t *pixels, const uint16_t pixelCount) {
        if (pixels == nullptr || pixelCount == 0)
            return;
        std::fill_n(pixels, pixelCount, 0);
    }

    inline void fillRgbPixels(uint32_t *pixels, const uint16_t pixelCount, const uint32_t color) {
        if (pixels == nullptr || pixelCount == 0)
            return;
        std::fill_n(pixels, pixelCount, color);
    }
    ...
    inline uint32_t ColorH(uint16_t hue) {
        uint8_t r, g, b;
        hue = (hue * 1530L + 32768) / 65536;
        if (hue < 510) {
            // Red to Green-1
        ...
        return (((r * 256) >> 8) * 256 & 0xff00) << 8 | ((g * 256) >> 8) * 256 & 0xff00 | (((b * 256) >> 8) * 256) >> 8;
    }
}
