...
{
public:
    MultiplexedStrip(uint16_t pin, uint16_t ledCount1, uint16_t ledCount2, uint16_t ledCount3, uint16_t ledCount4);
    void begin();
    ...
    void circle(uint8_t index, uint32_t lvl, uint32_t maxLvlAvg, uint16_t width, uint16_t bars, float moveSpeed, bool reverseOnPeak);
...
private:
    ...
    StripData *getCurrentStrip(uint8_t index);
